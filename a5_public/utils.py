...
def pad_sents_char(sents, char_pad_token):
    ...
    #     padded = []
    #     for word in sent:
    #         tmp = word[:max_word_length]
    #         tmp += [char_pad_token]*(max_word_length-len(tmp))
    for sent in sents:
        sent_padded = []
        # Pad / truncate each word accordingly.
        for word in sent:
            if len(word) < max_word_length:
                # Pad.
                padding = [char_pad_token] * (max_word_length - len(word))
                word.extend(padding)
            elif len(word) > max_word_length:
                # Truncate.
                word = word[0:max_word_length]
            sent_padded.append(word)
        # Pad sentence accordingly.
        if len(sent_padded) < max_sent_length:
            word_placeholder = [char_pad_token] * max_word_length
            padding = [word_placeholder] * (max_sent_length - len(sent_padded))
            sent_padded += padding
        # Store the newly padded sentence.
...
def pad_sents(sents, pad_token):
    ...
    max_length = max(sents, key=lambda s: len(s))
    for i,sentence in enumerate(sentences):
        sents_padded.append(sentence+[pad_token]*(max_length - len(sentence)))
