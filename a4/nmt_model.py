...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.encoder = nn.LSTM(input_size=embed_size, hidden_size=self.hidden_size, bias=True, bidrectional=True)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        enc_hiddens = pad_packed_sequence(enc_hiddens).permute(1,0,2)

        h_enc_cat = torch.cat([last_hidden[0], last_hidden[1]])
        init_decoder_hidden = self.h_projection(h_enc_cat)
        c_enc_cat = torch.cat([last_cell[0], last_cell[1]])
        init_decoder_cell = self.c_projection(c_enc_cat)
